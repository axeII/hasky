        ...
        self.context = {
            ...
            "+": ContextValue(
                ...
                Function(
                    "+",
                    None,
                    lambda x, y: self.control_values(x, y) + self.control_values(y, x),
                ),
            ),
            "-": ContextValue(
                "default",
            ...
            "stack": ContextValue(
                "default", Function("stack", None, lambda _: self.print_context_data())
            ),
            "type": ContextValue(
                "default",
                Function(
                    "type",
                    None,
                    lambda a: self.context[untoken(ast_value(a))].cont_val_data.name,
                ),
            ),
            "latex": ContextValue(
                "default",
    ...
    def interpret(self, data=None, ast=[]):
        ...
        def search_function(name_of_function):
            for searching_fn in list(
                ...
                if (
                    searching_fn.cont_val_data.name == "Function"
        ...
        def control_eval(fnd_func, context, set_args={}):
            def ctrlvar_scp(local_scope, global_scope, variable):
                ...
                if untoken(variable.value) in local_scope:
                    ...
                    return untoken(
                        ast_value(global_scope[untoken(variable.value)].cont_val_data)
                    )
                else:
                    return untoken(variable.value)

            for arg_number in range(len(fnd_func.cont_val_data.args)):
                set_args[untoken(fnd_func.cont_val_data.args[arg_number])] = untoken(
                    operation.value[arg_number].value
                )
            func_ = check_context(
                set_args, context, untoken(ast_value(fnd_func.cont_val_data.value[0]))
            )
            args_ = list(
                map(
            ...
            if key in self.context and operation.name != "CallingFunction":
                ...
                if found_fn.cont_val_type != "default":
                    ...
                    if isinstance(operation.value[0], List):
                        ...
                        data = list(
                            map(lambda x: untoken(x.value), operation.value[0].value)
                        )
